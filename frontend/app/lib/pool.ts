...
export async function fetchPoolInfo(token_address: string): Promise<PoolInfoResponse> {
    const url =
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/coins/find_pool?network=sui-network&token_address=` + token_address;
    ...
    const poolInfoBluefin = poolInfos.filter(pool => pool.dex_name === "bluefin");
